        ...
        def wrapped(self, *args, **kwargs):
            if not self._available_api_versions:
                ...
                if (
                    ...
                    or self.api_server == 'https://galaxy.ansible.com/'
                ):
                ...
                if 'available_versions' not in data:
                    raise AnsibleError(
                        "Tried to find galaxy API root at %s but no 'available_versions' are available "
                ...
                display.vvvv(
                    "Found API version '%s' with Galaxy server %s (%s)"
            ...
            if not common_versions:
                raise AnsibleError(
                    "Galaxy action %s requires API versions '%s' but only '%s' are available on %s %s"
                    % (
                        method.__name__,
                        ", ".join(versions),
...
def _urljoin(*args):
    return '/'.join(
        to_native(a, errors='surrogate_or_strict').strip('/') for a in args + ('',) if a
    )
...
class GalaxyError(AnsibleError):
    ...
    def __init__(self, http_error, message):
        ...
        if 'v2' in url_split:
            ...
            for error in errors:
                error_msg = (
                    error.get('detail') or error.get('title') or http_error.reason
                )
...
class CollectionVersionMetadata:
    def __init__(
        self, namespace, name, version, download_url, artifact_sha256, dependencies
    ):
        """
...
class GalaxyAPI:
    ...
    def __init__(
        ...
        display.debug(
            'Validate TLS certificates for %s: %s'
    ...
    def _add_auth_token(self, headers, url, token_type=None, required=False):
        ...
        if not self.token and required:
            raise AnsibleError(
                "No access token or username set. A token can be set with --api-key, with "
    ...
    def authenticate(self, github_token):
        ...
        """
        url = (
            _urljoin(self.api_server, self.available_api_versions['v1'], "tokens") + '/'
        )
    ...
    def create_import_task(
        self, github_user, github_repo, reference=None, role_name=None
    ):
        """
        Post an import request
        """
        url = (
            _urljoin(self.api_server, self.available_api_versions['v1'], "imports")
            + '/'
        )
        args = {
    ...
    def publish_collection(self, collection_path):
        ...
        """
        display.display(
            "Publishing collection artifact '%s' to %s %s"
    ...
    def wait_import_task(self, task_id, timeout=0):
        ...
        while timeout == 0 or (time.time() - start) < timeout:
            ...
            display.vvv(
                'Galaxy import process has a status of %s, wait %d seconds before trying again'
                % (state, wait)
            )
            time.sleep(wait)
        ...
        if state == 'waiting':
            raise AnsibleError(
                "Timeout while waiting for the Galaxy import process to finish, check progress at '%s'"
                % to_native(full_url)
            )
        ...
        for message in data.get('messages', []):
            ...
            if level == 'error':
                ...
                display.warning(
                    "Galaxy import warning message: %s" % message['message']
                )
            else:
                display.vvv(
                    "Galaxy import message: %s - %s" % (level, message['message'])
                )
        ...
        if state == 'failed':
            ...
            description = to_native(
                data['error'].get(
                    'description', "Unknown error, see %s for more details" % full_url
                )
            )
            raise AnsibleError(
    ...
    def get_collection_version_metadata(self, namespace, name, version):
        ...
        error_context_msg = (
            'Error when getting collection version metadata for %s.%s:%s from %s (%s)'
    ...
    def get_collection_versions(self, namespace, name):
        ...
        if 'v3' in self.available_api_versions:
            ...
            relative_link = (
                True  # AH pagination results are relative an not an absolute URI.
            )
        else:
        ...
        n_url = _urljoin(
            self.api_server, api_path, 'collections', namespace, name, 'versions', '/'
        )

        error_context_msg = (
            'Error when getting available collection versions for %s.%s from %s (%s)'
        ...
        while True:
            ...
            data = self._call_galaxy(
                to_native(next_link, errors='surrogate_or_strict'),
